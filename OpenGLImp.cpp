...
namespace ProcessPlanning
{
	...
	void OpenGLImp::center3DObject(ComplexGeometry* cg)
	{
		Point* c = cg->getSphereCenter();
		double sphereRadius = cg->getSphereRadius();
		double diam = 2 * sphereRadius;
		
		double zNear = 1.0;
		double zFar = zNear + diam;
		
		GLdouble left = c->getX() - diam;
		GLdouble right = c->getX() + diam;
		GLdouble bottom = c->getY() - diam;
		GLdouble top = c->getY() + diam;
	...
	void OpenGLImp::drawTriangle(Facet* t)
	{
		Point *v1, *v2, *v3;
		v1 = t->getV1();
		v2 = t->getV2();
	...
	void OpenGLImp::drawRect(Rect* r)
	{
		Point *v1, *v2;
